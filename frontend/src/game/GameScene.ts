...
export class GameScene extends Phaser.Scene {
  ...
  private static readonly DUNGEON_TRACKS = [
    'music-dungeon',
    'music-dungeon-2',
    'music-dungeon-3',
    'music-dungeon-4',
  ...
  private playMusic(key: string, loop = true) {
    const isDungeon = key === 'music-dungeon';
    // Don't interrupt dungeon music that's already playing — a new random track will
    // start automatically when the current one finishes.
    if (this.bgMusic && (this.bgMusic as Phaser.Sound.WebAudioSound).isPlaying) {
      if (isDungeon && GameScene.DUNGEON_TRACKS.includes(this.bgMusic.key)) return;
      if (!isDungeon && this.bgMusic.key === key) return;
    }
    const prevKey = this.bgMusic?.key;
    this.bgMusic?.stop();
    this.bgMusic?.destroy();
    this.bgMusic = null;
    const actualKey = isDungeon ? this.pickDungeonTrack(prevKey) : key;
    this.bgMusic = this.sound.add(actualKey, { loop: isDungeon ? false : loop, volume: this.masterVol * this.musicVol });
    if (isDungeon) {
      this.bgMusic.once('complete', () => this.playMusic('music-dungeon'));
    }
    (this.bgMusic as Phaser.Sound.WebAudioSound).play();
  }

  private pickDungeonTrack(excludeKey?: string): string {
    const pool = excludeKey
      ? GameScene.DUNGEON_TRACKS.filter(k => k !== excludeKey)
      : GameScene.DUNGEON_TRACKS;
  ...
  private syncEnemy(e: EnemyState, players: PlayerState[]) {
    ...
    if (!e.isAlive && !sp.dead) {
      ...
      if (e.name === 'Dark Mage' && sp.sprite) {
        ...
        this.playMusic('music-victory', false);
    ...
    // ── Alive: update position and animations ──
      ...
      if (sp.sprite) {
        ...
        if (isGolem) {
          ...
          sp.sprite.setAlpha(e.isUntargetable ? 0.4 : 1.0);
