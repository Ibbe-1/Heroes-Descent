...
// A fireball the Dark Mage boss has fired that is currently travelling through the room.
//
...
// Because it travels in a straight line toward where the player WAS when the boss fired,
...
public class ActiveProjectile
{
    ...
    // The fireball does NOT home in on the player; it flies in a straight line.
    // Moving out of this line after the shot is fired will dodge the damage.
    public float DirX { get; }
    public float DirY { get; }

    // How far the fireball has travelled since it was fired.
    ...
    // The fireball disappears after travelling this many pixels without hitting anyone.
    // Keeps it from flying off-screen if the player dodges.
    public const float MaxRange = 340f;

    // A player must be within this radius (px) of the fireball centre to be hit.
    // Generous enough to feel fair while rewarding players who move early.
    public const float HitRadius = 26f;

    // Damage applied on hit — copied from the boss's Attack stat when fired.
    // Stored here so it stays consistent even if the boss stat changes mid-flight.
    public int Damage { get; }

    public ActiveProjectile(float fromX, float fromY, float toX, float toY, int damage)
    {
        X      = fromX;
        Y      = fromY;
        Damage = damage;

        // Build the normalised direction from the boss's position toward the
        // player's position at the moment of firing.
        // len > 0 guard prevents a divide-by-zero if the boss is standing exactly
